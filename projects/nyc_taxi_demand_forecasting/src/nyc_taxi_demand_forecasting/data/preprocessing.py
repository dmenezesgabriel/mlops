...
class YellowTaxiTripPreprocessor:
    ...
    def preprocess(self, raw_directory: Path, output_directory: Path) -> Path:
        ...
        trips = pd.concat(self._read_raw_files(raw_directory), ignore_index=True)
    ...
    def clean(self, trips: pd.DataFrame) -> pd.DataFrame:
        ...
        selected_trips["duration_minutes"] = self._duration_minutes(selected_trips)
        valid_trips = selected_trips[selected_trips["duration_minutes"] > 0]
        valid_trips = valid_trips[valid_trips["PULocationID"] > 0]
        return valid_trips[valid_trips["trip_distance"] >= 0].reset_index(drop=True)

    def _read_raw_files(self, raw_directory: Path) -> list[pd.DataFrame]:
        parquet_paths = sorted(raw_directory.glob("*.parquet"))
        if parquet_paths:
            return [pd.read_parquet(path, columns=list(self._columns)) for path in parquet_paths]

        raise FileNotFoundError(f"Missing raw parquet files in {raw_directory}: expected *.parquet")
