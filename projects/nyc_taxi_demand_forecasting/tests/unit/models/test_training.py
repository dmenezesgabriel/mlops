...
def test_linear_demand_regressor_predicts_linear_target() -> None:
    # Arrange
    features = pd.DataFrame({"pickup_count": [1.0, 2.0, 3.0], "hour": [0.0, 0.0, 0.0]})
    target = pd.Series([3.0, 5.0, 7.0])

    # Act
    model = LinearDemandRegressor().fit(features, target)
    predictions = model.predict(pd.DataFrame({"pickup_count": [4.0], "hour": [0.0]}))
...
def test_demand_dataset_splitter_preserves_order() -> None:
    ...
    # Act
    train_frame, test_frame = DemandDatasetSplitter().split(dataset, test_size=0.4)
