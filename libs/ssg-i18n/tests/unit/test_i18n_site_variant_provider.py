...
def test_variants_create_localized_site_and_translated_sources(tmp_path: Path) -> None:
    # Arrange
    ...
    assert portuguese_variant.site.description == "Construa sistemas de machine learning."
...
def test_variants_translate_notebook_markdown_cells_without_code_cells(tmp_path: Path) -> None:
    # Arrange
    source_root = tmp_path / "content"
    source_root.mkdir()
    ...
    context = BuildContext(tmp_path / "site.yaml", tmp_path / "build", None, "test")
...
def test_variants_fall_back_to_source_text_when_machine_translation_drops_code_marker(
    ...
    # Assert
    translated_path = variants[1].site.collections[0].pages[0].source_path
    assert translated_path.read_text(encoding="utf-8") == "Use `MLflow` for tracking.\n"


def test_variants_translate_wikilink_labels_while_preserving_targets(tmp_path: Path) -> None:
    # Arrange
    source_root = tmp_path / "content"
    source_root.mkdir()
    source_path = source_root / "README.md"
    ...
    # Assert
    translated_path = variants[1].site.collections[0].pages[0].source_path
    assert translated_path.read_text(encoding="utf-8") == "Veja [[overview|Visao Geral]].\n"
