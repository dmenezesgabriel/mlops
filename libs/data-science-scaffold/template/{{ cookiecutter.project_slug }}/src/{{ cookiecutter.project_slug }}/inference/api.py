...
from {{ cookiecutter.project_slug }}.configuration import (
    ProjectConfig,
    ProjectConfigLoader,
)

app = FastAPI(title="{{ cookiecutter.project_title }} Serving API")
...
def fetch_online_features(location_id: int) -> pd.DataFrame:
    """Fetch recent demand features from the Feast online SQLite store.
