...
| QE-7 | [x] Server-side prepared-statement execution: `EXECUTE <name> [USING …]` + bind `ExecutionParameters` into the stored statement before Trino submit (wrangler ships the qmark placeholders + `ExecutionParameters`, `_utils.py:371-401`) | M | FR-11; wrangler `_statements.py` + tutorial CELL 27; moto: no EXECUTE support → PRD §3 row; aws UG `use-prepared-statements` + `use-execution-parameters` + launch blog (EXECUTE is pure SQL splicing, injection surface is the caller's); missing-statement/`?`-count mismatch → FAILED execution with Athena's exact `StateChangeReason`, bound statement's classification drives artifacts | QE-3, MD-3 |
